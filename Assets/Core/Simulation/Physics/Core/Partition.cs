...
namespace RTSLockstep
{
    public static class Partition
    {
        ...
        public const long Offset = (1 << ShiftSize) / 2; //Offset due to partition nodes being centered on grid positions
        ...
        private static int GridXMin, GridXMax, GridYMin, GridYMax;
        ...
        private static int ClampX(int gridX)
        {
            if (gridX < 0)
            {
                return 0;
        ...
        private static int ClampY(int gridY)
        {
            if (GridXMax < 0)
            {
                return 0;
