...
namespace RTSLockstep.Simulation.LSMath
{
    //THIS IS HAPPENING!!!!
    [System.Serializable]
    public struct Vector3d : ICommandData
    {
        ...
        {
            x = FixedMath.Create(vec3.x);
            y = FixedMath.Create(vec3.z);
            z = FixedMath.Create(vec3.y);
        }
        ...
        public override bool Equals(object obj)//Vector3d v1, Vector3d v2
        {
            return ((Vector3d)obj).x == x && ((Vector3d)obj).y == y && ((Vector3d)obj).z == z;
        }
